...
impl<'a> PluginRegistries<'a> {
    ...
    pub fn connect_cost_mut(&mut self) -> &mut PluginRegistry<'a, dyn EditConnectionCostPlugin> {
        &mut self.connect_cost
    }

    pub fn input_text_mut(&mut self) -> &mut PluginRegistry<'a, dyn InputTextPlugin> {
        &mut self.input_text
    }

    pub fn oov_mut(&mut self) -> &mut PluginRegistry<'a, dyn OovProviderPlugin> {
        &mut self.oov
    }

    pub fn path_rewrite_mut(&mut self) -> &mut PluginRegistry<'a, dyn PathRewritePlugin> {
        &mut self.path_rewrite
    }

    pub fn load<'b>(&mut self, grammar: &'a mut Grammar<'b>) -> SudachiResult<()> {
...
impl<'a> From<PluginRegistries<'a>> for PluginContainers {
    fn from(value: PluginRegistries) -> PluginContainers {
        PluginContainers {
            connect_cost: value.connect_cost.into(),
            input_text: value.input_text.into(),
            oov: value.oov.into(),
            path_rewrite: value.path_rewrite.into(),
        }
    }
}
